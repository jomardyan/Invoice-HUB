...
function NotificationCenterRealtime() {
    ...
    useEffect(() => {
        const cleanup = socket.onNotification(handleNewNotification);
        return cleanup;
    }, [socket, handleNewNotification]);

    // Listen to invoice events for automatic notification updates
    useEffect(() => {
        const cleanupInvoicePaid = socket.onInvoicePaid(() => refetch());
        const cleanupInvoiceSent = socket.onInvoiceSent(() => refetch());
        const cleanupPayment = socket.onPaymentReceived(() => refetch());
    ...
    return (
        <>
            <Tooltip title={isConnected ? 'Real-time updates active' : 'Offline mode'}>
                <IconButton
                    color="inherit"
                    onClick={handleClick}
            ...
            >
                <Box sx={{ p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        {isConnected && (
                            <Chip label="Live" size="small" color="success" sx={{ fontSize: '0.7rem' }} />
                        )}
                    </Box>
                    {unreadCount > 0 && (
...
export default NotificationCenterRealtime;
